...
class session {
    ...
    static function initByInstance($instance){
        self::$instance = $instance;
        session::init();
    }
    
    public function getSessionTable() {
        return self::$instance.".".LOVE_WS_SESSIONS;
    }

    public function establishDbConnection(){
        $link = mysql_connect(DB_SALES_SERVER, DB_SALES_USER, DB_SALES_PASSWORD) or die('Could not connect: ' . mysql_error());
        if($link === null){
    ...
    function read($sessID) {
        $db = $this->getDbHandle();
        // fetch session-data
        $res = mysql_query("SELECT session_data AS d FROM ".$this->getSessionTable()."
                            WHERE session_id = '".mysql_real_escape_string($sessID)."'
                            AND session_expires > ".time(),$db) or error_log("read session: ".mysql_error());
    ...
    function write($sessID,$sessData) {
        ...
        $res = mysql_query("SELECT * FROM ".$this->getSessionTable()."
        ...
        if($res && mysql_num_rows($res)) {
            ...
            mysql_query("UPDATE ".$this->getSessionTable()."
        ...
        // if no session-data was found,
            ...
            // create a new row
            mysql_query("INSERT INTO ".$this->getSessionTable()." (
                         session_id,
                         session_expires,
                         session_data)
                         VALUES(
    ...
    function destroy($sessID) {
        $db = $this->getDbHandle();
        // delete session-data
        mysql_query("DELETE FROM ".$this->getSessionTable()." WHERE session_id = '".mysql_real_escape_string($sessID)."'",$db);
    ...
    function gc($sessMaxLifeTime) {
        $db = $this->getDbHandle();
        // delete old sessions
        mysql_query("DELETE FROM ".$this->getSessionTable()." WHERE session_expires < ".time(),$db);
