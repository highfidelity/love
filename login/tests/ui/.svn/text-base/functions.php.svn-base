...
function generateSuiteReport($suite, $buffer, $result)
{
    ...
    foreach ($result->failures() as $test) {
        $failedTests[] = array(
            'name'    => $test->failedTest()->getName(),
            'class'   => get_class($test->failedTest()),
            'details' => str_replace('<', '&lt;', $test->getExceptionAsString()) .
...
EOF;
        ...
        echo <<<EOF
                    (<a href="javascript: controlSuite('$suiteId', true);">run only</a>|<a href="javascript: controlSuite('$suiteId', false);">exclude</a>)
                </span>
            </div>
...
EOF;
        ...
        foreach ($suiteReport['failedTests'] as $test) {
            printf(
                "<p class=\"testcase_fail\">TestCase <a class=\"testcase\" href=\"javascript: controlSuite('%s', true, '%s');\"> <span class=\"suite\">%s</span>-&gt;<span class=\"case\">%s()</span> </a>: <span class=\"fail\">failed</span><span class=\"exception\"><div><pre>%s</pre></div></span></p>\n",
                $suiteId,
                $suiteId . '@' . $test['name'],
                $test['class'],
                $test['name'],
