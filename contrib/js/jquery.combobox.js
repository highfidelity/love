...
;(function($) {
	var ComboBox = Class.create({
		...
		init: function(o) {
            ...
            var list = this.list,
                oThis=this;
            $("option",this.el).bind( 'remove', function(event){
                $("li[val="+$(this).val()+"]",list).remove();
                $("li[val=0]",list).addClass("ui-combobox-list-selected");
                oThis.textbox.val($("li[val=0]",list).text());
            });
		},
		...
		click: function(e) {
			...
			if (this.list.is(":visible")) {
                ...
                off = this.container.offset();
                winTop = $(window).scrollTop();
                winHeight = $(window).height();
                scrollHeight = this.container.outerHeight(true) + this.list.outerHeight(true);
                if ( off.top+scrollHeight > winTop+winHeight ) {
                    $(window).scrollTop( off.top+scrollHeight -winHeight);
                }                            
			}
			return false;
		},
