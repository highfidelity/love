    ...
    return {
        ...
        drawDots:function (fAfter) {
            ...
            for (var i = 0, o = 0; i < this.interpolated.length; i++) {
                ...
                if (i == len) {
                    if (fAfter) fNewAfter = fAfter;
                }
                this.dots[o].show().animate({ cx:x, cy:y }, this.chart.animationLength, '<>', fNewAfter);
                o++;
            }
        ...
        setData:function (values, dontInterpolate, fAfter) {
            this.values = values;
...
TimelineChart.prototype.getSeriesMaxFloat = function (seriesData, initialMax) {
    var max = initialMax;
    for (var i = 0; i < seriesData.length; i++) {
        var dataValue = seriesData[i], intValue = 0;
        if(dataValue == undefined) {
...
TimelineChart.prototype.normalizeSeriesFloat = function (seriesData, maxValue) {
    var normalizedData = [];
    for (var i = 0; i < seriesData.length; i++) {
        var dataValue = seriesData[i], intValue = 0;
        if(dataValue == undefined) {
         ...
         // intValue = parseInt(dataValue,10);
          normalizedData[i] = dataValue / maxValue;
        }
    }
    return normalizedData;
};
...
TimelineChart.prototype.setBottomLabels = function (attrs, labels) {
    this.setAxisLabelsAreaHeight(45);//labels.length * 30 + 30);
    ...
    for (var i = 0; i < labels.length; i++) {
        var y = this.height - this.axisLabelsAreaHeight + (i + 1) * 20;
        var row = labels[i];

        var minX = -1000, spacing = 5;
...
LoveChart = {
    ...
    sendersFill:null,
    feeCountFill:null,
    teamFeeCountFill:null,

    initialize:function (containerId, width, height, samples, amountInDollar) {
        ...
        this.teamFeeCountFill = this.chart.addFillSeries({ fill:'#44AAFB', opacity:0.3 });
        ...
        $(this.chart.r.canvas).mousemove( function (e) { 
            ...
            var coords = dots.findNearestPoint(e.pageX - $('#' + containerId).offset().left + $('#' +containerId).scrollLeft());
    ...
    load:function (data,fAfter) {
        if ( !data.points || data.points === null || data.points.length == 0 ) {
            return;
        }
        var prorataText = ' from all users or team members';
        var amount = this.amountInDollar;
        var leftMax = LoveChart.chart.getSeriesMaxFloat(data.percentage,5);
        var leftPoints = LoveChart.chart.getSeriesMaxFloat(data.percentage,5);
        var rightMax = LoveChart.chart.getSeriesMax(data.givers,5);
        rightMax = LoveChart.chart.getSeriesMax(data.givers,rightMax);
        var senders = [], messages = [], feeCount = [], teamFeeCount = [];

        messages = LoveChart.chart.normalizeSeriesFloat(data.percentage, leftPoints);
        senders = LoveChart.chart.normalizeSeries(data.users, rightMax);
        feeCount = LoveChart.chart.normalizeSeries(data.givers, rightMax);
        teamFeeCount = LoveChart.chart.normalizeSeries(data.team_givers, rightMax);

        var labels = [];
        var fullLabels = data.users;
        var month, year;
        var nonEmptyDateLabels = 0, nonEmptyWeekLabels = 0, nonEmptyMonthLabels = 0, nonEmptyYearLabels = 0;
        labels.push(data.users);

        LoveChart.chart.setBottomLabels({ fill:'black', font:'9px Arial, sans-serif' }, labels);
        LoveChart.chart.setLeftLabels({ fill:'#F4B645', font:'11px Arial, sans-serif', 'text-anchor':'end' }, leftPoints, 100);
        ...
        var teamFeeCountFillData = [].concat(teamFeeCount);
        teamFeeCountFillData[0] = teamFeeCount[0] || 0;
        teamFeeCountFillData[teamFeeCountFillData.length - 1] = teamFeeCount[teamFeeCount.length - 1] || 0;
        var range = 'Day';
        ...
        LoveChart.teamFeeCountFill.setData(teamFeeCountFillData, true);
        LoveChart.messagesDots.setData(messages, true,fAfter);
        ...
        LoveChart.chart.popupTextCallback = function (xs) {
	        ...
	        var selectedTeamFeeCountValue = teamFeeCount[o];
	        if(selectedTeamFeeCountValue == undefined) {
	            selectedTeamFeeCountValue = teamFeeCount[o - 1] || 0;
	        }
	        var dateRangeText = fullLabels[o];
            /***
            The option used to populate the data is not saved in the database, so we need to find other tips 
            to display the correct message.
            The first way to get the info is to look at the radio buttons (simulationLovesReceived and simulationLovesReceivedNormalized)
            The second way to get the info is to compare the number of Loves and the number of Givers, 
            if  #Loves <  #Givers, it means that the graph has been built using the prorata normalized
            In all the other cases, we cannot know which option has been used at populate time. 
            ***/
            if ($('.simulationLovesReceived').attr('checked')) {
                prorataText = ' from all users';
            } else if ($('.simulationLovesReceivedNormalized').attr('checked')) {
                prorataText = ' from team members';
            } else if (Math.round(data.points[o]) < Math.round(selectedFeeCountValue * rightMax)) {
                prorataText = ' from team members';
            }
            
	        return [
	            { fill:'#F4B645', font:'bold 11px Arial, sans-serif', text:' Recommended $ : '+ formatAsCurrency( parseInt(amount*selectedMessageValue * leftMax)/100+'') + 
                    '  ' },
	            { fill:'#00FFFB', font:'bold 11px Arial, sans-serif', text:' Givers: ' + Math.round(selectedFeeCountValue * rightMax) + ' ' },
	            { fill:'#44AAFB', font:'bold 11px Arial, sans-serif', text:' Team Givers: ' + Math.round(selectedTeamFeeCountValue * rightMax) + ' ' },
	            { fill:'black', font:'bold 11px Arial, sans-serif', text: dateRangeText },
	            { fill:'#000044', font:'bold 11px Arial, sans-serif', text:prorataText },
	            { fill:'#000044', font:'bold 11px Arial, sans-serif', text:' # Loves: ' + Math.round(data.points[o]) + ' ' },
	            { fill:'#000044', font:'bold 11px Arial, sans-serif', text:' Values of Loves: ' + (Math.round(100*data.percentage[o])/100) + ' %' }
	        ];
        };
    },
