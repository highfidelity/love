...
LoveChart = {
    ...
    load:function (from, to, username) {
	   ...
	   var rightMax = 5, leftMax = LoveChart.chart.getSeriesMax(data.messages,5);
	   ...
	   senders = LoveChart.chart.normalizeSeries(data.senders, leftMax);
		...
		var dateRangeText = fullLabels[o];
                ...
                return [
                    { fill:'#F4B645', font:'bold 11px Arial, sans-serif', text:' Total love: '+ Math.round(selectedMessageValue * leftMax) + ' ' },
                    { fill:'#00F01B', font:'bold 11px Arial, sans-serif', text:' Unique '+LoveChart.userType + (Math.round(selectedSendersValue * leftMax) == 1 ? '' : 's') + ': '+ Math.round(selectedSendersValue * leftMax) + ' '  },
